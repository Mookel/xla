    ...
    switch (instruction->opcode()) {
      case HloOpcode::kSend: {
        ...
        if (group.empty()) {
          ...
          CHECK_EQ(group[0]->opcode(), HloOpcode::kRecvDone);
          CHECK(dependencies.insert({group[0], {instruction.get()}}).second);
        }
        break;
      }
      case HloOpcode::kRecvDone: {
        ...
        if (group.empty()) {
          ...
          CHECK_EQ(group[0]->opcode(), HloOpcode::kSend);
          CHECK(dependencies.insert({instruction.get(), {group[0]}}).second);
        }
        break;
      }
