...
class HloCostAnalysis : public ConstDfsHloVisitor {
 public:
  ...
  //
  // Note that unlike a regular hashtable, there's no notion of an "unuset" key.
   ...
   public:
    ...
    float& operator[](absl::string_view property) {
      if (property == kFlopsKey) {
        return flops_;
      } else if (property == kTranscendentalsKey) {
        return transcendentals_;
      } else if (property == kBytesAccessedKey) {
        return bytes_accessed_;
      } else if (property == kOptimalSecondsKey) {
        return optimal_seconds_;
      } else if (property == kUtilizationKey) {
        return utilization_;
      } else if (property == kOperand0UtilizationKey) {
        return operand0_utilization_;
      } else if (property == kOperand1UtilizationKey) {
        return operand1_utilization_;
      } else if (property == kOperand0BytesAccessedKey) {
        return operand0_bytes_accessed_;
      } else if (property == kOperand1BytesAccessedKey) {
        return operand1_bytes_accessed_;
      } else if (property == kOutputRootBytesAccessedKey) {
        return output_root_bytes_accessed_;
      } else if (property == kReserved0Key) {
        return reserved0_;
      } else if (property == kReserved1Key) {
        return reserved1_;
      } else {
    ...
    float operator[](absl::string_view property) const {
      if (property == kFlopsKey) {
        ...
        if (it != named_props_.end()) return it->second;
        return 0;
      }
    }
